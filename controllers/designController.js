import Design from '../models/desinModel.js';
import mongoose from 'mongoose';


// Get all designs
const getAllDesigns = async (req, res) => {
  try {
    const designs = await Design.find();
    res.status(200).json({
      data: designs,
      message: 'success',
      status: 200
    });
  } catch (error) {
    res.status(400).json({
      data: null,
      message: error.message,
      status: 400
    });
  }
};

// Get a single Design
const getDesign = async (req, res) => {
  const { id } = req.params;

  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json({
      data: null,
      message: 'not found',
      status: 404
    });
  }
  const design = await Design.findById(id);

  if (!design) {
    return res.status(404).json({
      data: null,
      message: 'not found',
      status: 404
    });
  }

  res.status(200).json({
    data: design,
    message: 'success',
    status: 200
  });
}

// Create a new Design
const createDesign = async (req, res) => {
  const { images, companyId, categoryId } = req.body;
  try {
    const imagesArray = req.files.map(file => file.filename);
    const design = await Design.create({ images: imagesArray, companyId, categoryId });

    res.json({
      data: design,
      message: 'success',
      status: 200
    });
  } catch (err) {
    res.status(400).json({
      data: null,
      message: err.message,
      status: 400
    });
  }
};

// Update a Design
const updateDesign = async (req, res) => {
  const { id } = req.params;

  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json({
      data: null,
      message: 'not found',
      status: 404
    });
  }
  const { images, companyId, categoryId } = req.body;
  try {
    const imagesArray = req.files.map(file => file.filename);
    const updatedDesign = await Design.findByIdAndUpdate(id, { images: imagesArray, companyId, categoryId }, { new: true });

    if (!updatedDesign) {
      return res.status(404).json({
        data: null,
        message: 'not found',
        status: 404
      });
    }

    res.status(200).json({
      data: updatedDesign,
      message: 'success',
      status: 200
    });
  } catch (error) {
    res.status(400).json({
      data: null,
      message: error.message,
      status: 400
    });
  }
};

// Delete a Design
const deleteDesign = async (req, res) => {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json({
      data: null,
      message: 'not found',
      status: 404
    });
  }
  const design = await Design.findByIdAndDelete(id);

  if (!design) {
    return res.status(404).json({
      data: null,
      message: 'not found',
      status: 404
    });
  }

  res.status(200).json({
    data: design,
    message: 'success',
    status: 200
  });
};

// Export functions
export { createDesign, getAllDesigns, getDesign, deleteDesign, updateDesign };
